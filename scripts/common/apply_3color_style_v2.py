...
원본 shapefile을 사용하여 3색 스타일 적용 (초록/파랑/빨강) + 노란 테두리
"""
...
with open('C:/Users/ksj27/PROJECTS/QGIS/input/green_list.txt', 'r', encoding='utf-8') as f:
    green_list = [line.strip() for line in f if line.strip()]

with open('C:/Users/ksj27/PROJECTS/QGIS/input/blue_list.txt', 'r', encoding='utf-8') as f:
    blue_list = [line.strip() for line in f if line.strip()]

with open('C:/Users/ksj27/PROJECTS/QGIS/input/red_list.txt', 'r', encoding='utf-8') as f:
    red_list = [line.strip() for line in f if line.strip()]

print(f"📋 지번 목록 읽기 완료:")
print(f"  🟢 초록: {len(green_list)}개")
print(f"  🔵 파랑: {len(blue_list)}개")
print(f"  🔴 빨강: {len(red_list)}개")
...
# 2. 원본 shapefile 로드
shp_path = 'C:/Users/ksj27/PROJECTS/QGIS/data/haengwonri_all.shp'
...
if not layer.isValid():
    ...
    print(f"✅ 레이어 로드 성공 (전체 {layer.featureCount()}개 필지)")
    ...
    # 초록색 규칙 (제주시추천 + 국공유지)
    green_filter = '"A5" IN (\'' + '\',\''.join(green_list) + '\')'
    ...
    blue_rule.setFilterExpression(blue_filter)
    blue_rule.setLabel('일반 사유지')
    root_rule.appendChild(blue_rule)

    # 빨간색 규칙 (기개발 사유지)
    red_filter = '"A5" IN (\'' + '\',\''.join(red_list) + '\')'
    ...
    label_filter = '"A5" IN (\'' + '\',\''.join(all_jibun) + '\')'
    ...
    if border_layer.isValid():
        # 해당 필지만 선택하는 필터
        border_filter = '"A5" IN (\'' + '\',\''.join(all_jibun) + '\')'
        border_layer.setSubsetString(border_filter)
