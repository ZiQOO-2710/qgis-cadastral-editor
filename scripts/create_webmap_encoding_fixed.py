"""
서초구 지적도 + 아파트 웹맵 (인코딩 완전 해결 버전)
- DBF 로드 시 인코딩 명시적 지정: EUC-KR
"""
...
# 인코딩을 명시적으로 지정: EUC-KR
apt_layer = QgsVectorLayer(apt_path + '|encoding=EUC-KR', 'temp', 'ogr')

if not apt_layer.isValid():
    print("   ⚠️  EUC-KR 실패, CP949 시도...")
    apt_layer = QgsVectorLayer(apt_path + '|encoding=CP949', 'temp', 'ogr')
