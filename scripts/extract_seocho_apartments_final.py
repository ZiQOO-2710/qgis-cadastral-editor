"""
실거래가 CSV 기반 서초구 아파트 웹맵 생성 (최종 버전)
...
if len(transactions_by_complex) == 0:
    print("\n   ⚠️  서초구 거래 데이터가 없습니다!")
    raise Exception("CSV에 서초구 데이터가 없습니다")
...
print(f"   ✅ {len(apt_features)}개 아파트")

# 5단계: 지적도 GeoJSON (기존 재사용 또는 새로 생성)
print("\n5️⃣  지적도 GeoJSON 확인 중...")
...
if not os.path.exists(cadastral_geojson_path):
    ...
    if cadastral_layer.isValid():
        ...
        print("   ⚠️  지적도 레이어 로드 실패")
else:
    print(f"   ✅ 기존 파일 사용: {cadastral_geojson_path}")
...
required_files = [
    ('apartments_with_prices.geojson', '아파트 GeoJSON'),
    ('cadastral.geojson', '지적도 GeoJSON'),
    ('index.html', 'HTML 파일')
]
...
if not all_ok:
    raise Exception("필수 파일이 생성되지 않았습니다")
