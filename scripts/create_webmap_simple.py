"""
서초구 지적도 + 아파트 웹맵 (단순 버전)
- 아파트는 이미 EPSG:4326이므로 변환 불필요
- 지적도만 좌표 변환
"""
...
if apt_layer.isValid():
    ...
    for i, feature in enumerate(apt_layer.getFeatures()):
        ...
        apt_features.append({
            ...
            'properties': {
                'apt_nm': str(feature['apt_nm'] or ''),
                'rdnmadr': str(feature['rdnmadr'] or ''),
                'dngct': int(feature['dngct']) if feature['dngct'] else 0
            }
        })
